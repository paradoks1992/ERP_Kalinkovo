...
import { getStorages, getProductTypes, getProducts, inbound, progressTask } from "../lib/api";
...
export default function InboundForm() {
  ...
  const doInbound = async () => {
    try {
      ...
      if (
        ...
        try { await progressTask(activeTask.id, payload.quantity, "Автоучёт из формы ввоза"); } catch {}
      }

      setConfirm(false);
      toast.success(res?.queued ? "Операция поставлена в очередь (оффлайн)" : "Ввоз выполнен");
    } catch {
      setConfirm(false);
  ...
  const selectedStorage = storages.find((s) => String(s.id) === String(values.storageId));
  ...
  return (
    <div className="card">
      ...
      <ConfirmModal open={confirm} onCancel={() => setConfirm(false)} onConfirm={doInbound} confirmText="ОК">
...
function Field({ label, children }) {
  return (<label className="block"><div className="label">{label}</div>{children}</label>);
}
function Row({ k, v }) {
  return (<div className="flex justify-between gap-3 text-sm"><div className="text-slate-500">{k}</div><div className="font-medium text-slate-800">{v}</div></div>);
}
