          ...
          <div key={t.id} className={`p-3 rounded-xl2 border ${activeTask?.id === t.id ? "border-brand-400 bg-brand-50" : "border-slate-200 bg-white"}`}>
            <div className="flex items-center justify-between">
              <div className="font-medium">
                #{t.id} — {t.type === "inbound" ? "Ввоз" : "Вывоз"}: {t.productName} ({t.productTypeName}{t.category ? `/кат.${t.category}` : ""})
              </div>
              <div className="text-sm text-slate-600">Осталось: <b>{t.remaining}</b></div>
            </div>
            ...
            <div className="mt-2 flex gap-2">
              {!t.accepted && <button className="px-3 py-1.5 rounded-lg border border-slate-300 bg-white hover:bg-brand-50" onClick={() => accept(t.id)}>Принять</button>}
              ...
              <button className="px-3 py-1.5 rounded-lg border border-slate-300 bg-white hover:bg-green-50" onClick={() => complete(t.id)}>Выполнена</button>
            </div>
          </div>
        ))}
      </div>
